    ...
    if(isset($_GET['update'])) {
        ...
        if(isset($_FILES['logo2']) && $_FILES['logo2']['error'] === UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($fname2, $targetDir . $newfname2)) {
                ...
                $updateSql = "UPDATE product_category SET image = '$newfname2', name = '$name' WHERE id = '$imageid'";
            } else {
                ...
                Swal.fire({
                    ...
                    window.location.href = 'contentview.php?update=$imageid';
                });
                </script>";
                exit();
            }
            ...
            $updateSql = "UPDATE product_category SET name = '$name' WHERE id = '$imageid'";
        }
        ...
        if ($conn->query($updateSql)) {
            ...
            Swal.fire({
                ...
                window.location.href = 'contentview.php';
            });
            </script>";
        } else {
            echo "<script>
        ...
        <div class="card m-b-30">
            ...
            <div class="card-body">
                <?php
                        include("config.php");
                        $imageid = $_GET['update'];
                        $productQuery = "SELECT * FROM product_category WHERE id = '$imageid'";
                        $productResult = $conn->query($productQuery);

                        if ($productResult->num_rows > 0) {
                            $row = $productResult->fetch_assoc();

                            $aimage = 'product_category/' . $row['image'];
                            $name =  $row['name'];
                        }
                        ?>
                <form method="POST" enctype="multipart/form-data" class="p-5">
                    <div class="position-relative form-group">
                        <label for="productImage" class="form-label">Product Category Name</label>
                        <input type="text" class="form-control" id="productImage" name="name"
                            value="<?php echo $name; ?>">
                    </div>
                    <div class="position-relative form-group">
                        <label for="productImage" class="form-label">Product Category Image</label>
                    ...
                    <a href="contentview.php" class="btn btn-primary">View</a>
                </form>
