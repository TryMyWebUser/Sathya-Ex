...
if (isset($_POST["submit"])) {
    ...
    if (!empty($name)) {
        if (!empty($id)) {
            $updateSql = "UPDATE about_pagelsit SET name = '$name' WHERE list_id = $id";
            ...
            $insertSql = "INSERT INTO about_pagelsit (name) VALUES ('$name')";
    ...
    echo "<script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = 'list2.php?id={$id}&confirmed_delete=true';
                } else {
...
if(isset($_GET['confirmed_delete']) && $_GET['confirmed_delete'] == 'true'){
    $id = $_GET['id'];
    $deleteSql = "DELETE FROM about_pagelsit WHERE list_id = $id";
...
if (isset($_GET['id'])) {
    ...
    $selectSql = "SELECT * FROM about_pagelsit WHERE list_id = $id";
