...
if ($result->num_rows > 0) {
                    ...
                    <h2 class="title">
                        <?php
                            ...
                            if(isset($_GET["id"]) && is_numeric($_GET["id"])) {
                                $categoryId = $_GET["id"];
                                $stmt = $conn->prepare("SELECT * FROM prodsubcat WHERE subcat_id = ?");
    ...
    <section class="blog__post-area-three section-py-120">
        <div class="container">
            ...
            if(isset($_GET["id"]) && is_numeric($_GET["id"])) {
                $categoryId = $_GET["id"];
                $stmt = $conn->prepare("SELECT * FROM products WHERE subcat_id = ?");
                $stmt->bind_param("i", $categoryId);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<a href='primers.php?id=" . $row['prod_id'] . "' class='btn btn-two'>Read Details</a>";
                        echo "</div>";
                        echo "</div>";
                        echo "</div>";
                    }
